...
export default function App() {
  ...
  const SearchScreen = () => {
    ...
    useEffect(()=>{
      ...
      const loadRatings = async () => {
        ...
        for(const v of vendors) {
          try {
            const r = await getVendorRating(v.id);
            ratings[v.id] = r;
          } catch(e){}
        }
        setVendorRatings(ratings);
      };
      if(vendors.length) loadRatings();
    },[vendors.length]);
    ...
    const filteredVendors = vendors.filter(v=>{
      ...
      return cityOk && catOk && nameOk && ratingOk;
    });
      ...
      <div style={{paddingBottom:70}}>
        ...
        <div style={{display:"flex",background:T.card,borderBottom:`1px solid ${T.border}`}}>
          {[{id:"produits",label:`Produits (${filtered.length})`},{id:"vendeurs",label:`Vendeurs (${filteredVendors.length})`}].map(tab=>(
            <button key={tab.id} style={{flex:1,padding:"12px",background:"none",border:"none",borderBottom:`3px solid ${searchTab===tab.id?T.orange:"transparent"}`,cursor:"pointer",fontSize:14,fontWeight:600,color:searchTab===tab.id?T.orange:T.sub}}
        ...
        {searchTab==="vendeurs"&&(
          <div style={{background:T.card,padding:"10px 12px",borderBottom:`1px solid ${T.border}`}}>
            ...
            <div style={{display:"flex",gap:6,overflowX:"auto",scrollbarWidth:"none",marginBottom:8}}>
              {cities.map(c=>(
                <button key={c} style={{background:cityFilter===c?T.orange:T.tag,color:cityFilter===c?"#fff":T.sub,border:"none",borderRadius:20,padding:"5px 12px",fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}
            ...
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              ...
              {[0,3,4,5].map(r=>(
                <button key={r} style={{background:ratingFilter===r?T.orange:T.tag,color:ratingFilter===r?"#fff":T.sub,border:"none",borderRadius:20,padding:"5px 10px",fontSize:12,fontWeight:600,cursor:"pointer"}}
              ...
              :<div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {[
                    ...
                    <button key={cat.id} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:8,padding:"14px 12px",display:"flex",alignItems:"center",gap:10,cursor:"pointer",color:T.text,fontSize:14,fontWeight:500}}
                      onClick={()=>{setCategory(cat.id);go("home");}}>
          ...
          {searchTab==="vendeurs"&&(
            filteredVendors.length===0
              ...
              :filteredVendors.map(v=>{
                ...
                return (
                  <div key={v.id} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:14,marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:12}}
                    ...
                    <div style={{width:52,height:52,borderRadius:"50%",background:v.color||T.orange,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:18,flexShrink:0,overflow:"hidden"}}>
                      {v.logo_url
                        ?<img src={v.logo_url} style={{width:52,height:52,objectFit:"cover"}}/>
                        :<span>{v.initials||v.name?.[0]}</span>
                      }
                    </div>
      ...
      <div style={{paddingBottom:70,animation:"fadeIn 0.3s ease"}}>
        ...
        <div style={{padding:16}}>
          ...
          {[
            ...
            {icon:<CalendarDays size={20}/>,label:"Mes rendez-vous",action:()=>go("my-appointments"),delay:"0.17s"},
        ...
        <div style={{padding:"12px 16px"}}>
          {loading
            ...
            ?<div style={{textAlign:"center",padding:"60px 20px"}}>
              ...
              <div style={{fontSize:13,color:T.sub,marginBottom:20}}>Prenez RDV avec un vendeur certifié.</div>
              <button style={{background:T.orange,color:"#fff",border:"none",borderRadius:10,padding:"12px 24px",fontSize:14,fontWeight:700,cursor:"pointer"}} onClick={()=>go("home")}>Explorer le marché</button>
            </div>
            :myAppts.map(a=>(
              <div key={a.id} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:14,marginBottom:10}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:8}}>
                  ...
                  <span style={{
                    background:a.status==="confirmed"?"#E8F5E9":a.status==="cancelled"?"#FFEBEE":"#FFF3E0",
                    color:a.status==="confirmed"?"#2E7D32":a.status==="cancelled"?"#E53935":"#E65100",
                    borderRadius:20,padding:"4px 10px",fontSize:11,fontWeight:700,whiteSpace:"nowrap",marginLeft:8
                  }}>
                    {a.status==="confirmed"?"✅ Confirmé":a.status==="cancelled"?"❌ Annulé":"⏳ En attente"}
                  </span>
                </div>
                ...
                {a.status==="pending"&&(
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button style={{flex:1,background:"#FFEBEE",color:"#E53935",border:"none",borderRadius:8,padding:"8px",fontSize:12,fontWeight:700,cursor:"pointer"}}
  ...
  const screens = {home:HomeScreen,search:SearchScreen,product:ProductScreen,vendor:VendorScreen,cart:CartScreen,booking:BookingScreen,dashboard:DashboardScreen,profile:ProfileScreen,tos:TosScreen,privacy:PrivacyScreen,'vendor-request':VendorRequestScreen,admin:AdminScreen,favorites:FavoritesScreen,'my-appointments':MyAppointmentsScreen};
